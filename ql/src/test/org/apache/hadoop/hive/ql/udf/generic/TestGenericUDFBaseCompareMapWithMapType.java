...
public class TestGenericUDFBaseCompareMapWithMapType {
  
  private final BinaryUDFCall call;

  public TestGenericUDFBaseCompareMapWithMapType(BinaryUDFCall call) {
    this.call = call;
  }
  ...
  @Test
  public void testInitialiseOnEqualityOpSucceeds() throws UDFArgumentException {
    ...
    for (GenericUDF u : udfs) {
      u.initialize(new ObjectInspector[] { call.left, call.right });
    }
  }
  
  @Test
  public void testInitialiseOnNonEqualityOpThrowsException() {
    List<GenericUDF> udfs = Arrays.asList(
    ...
    for (GenericUDF udf : udfs) {
      try {
       udf.initialize(new ObjectInspector[] { call.left, call.right });
